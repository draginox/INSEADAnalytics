...
## NOTES
# if you see abs(correlation)>0.5 - you should think about it closely
```
...
## NOTES
# eigenvalue is SUM(# of inputs) = # of inputs (i.e., 7)
# when you check component 1 eigenvalue / sum of total eigenvalues = %age of explained var
# do you need to use non-collinear methods to start?
# no, BUT in this case we are ASSUMING that the inputs are non-collinear
...
## NOTES
# what is the stopping point for the components that you keep vs discard? 
# we are trying to balance, but how? Three ways, BUT check with biz rule & common sense:
# 1) Pick the components with eigenvalue > 1 ==> punch above weight
# 2) Managerial requirement for how much variance you want explained ==> don't care beyond X
# 3) Scree plot: stop to the left of elbow in scree plot (marginal contrib is small after X)
# COMBINE all of these with intuition and judgment to select
```
...
```{r}
## NOTES
# Varimax tends to do reasonably well
...
## NOTE
# Component 1 and Component 2 outputs note the amount of weight on that variable specifically
```
...
## NOTES
# no change to the values, just suppressed some low loading values
# communication loads on both components
# useful properties of PCA:
# 1) each component loads on SOME of the originally provided features
# 2) each original feature only factors into ONE* component
# 3) each component is separable from the other (DOUBLE CHECK THIS)
```
...
## NOTES
# We now only have 2 columns instead of 7, woohoo!
```
